...
if menu == "Agenda":
    ...
    for i, d in enumerate(dias_semana):
        with header_cols[i]:
            st.markdown(f"<div style='text-align:center; color:#888; font-size:12px; font-weight:600; padding:4px;'>{d}</div>", unsafe_allow_html=True)

    # Semanas
    for semana in cal:
        sem_cols = st.columns(7)
        for i, dia_num in enumerate(semana):
            with sem_cols[i]:
                ...
                if es_domingo:
                    bg = "#1a1a1a"; brd = "#333"; txt = "#555"
                elif es_festivo:
                    bg = "#2a0a0a"; brd = "#ff4444"; txt = "#ff6666"
                elif num_citas > 0:
                    bg = "#0a2a1a"; brd = "#44ff88"; txt = "#44ff88"
                else:
                    bg = "#1a1a0a"; brd = "#C9A84C"; txt = "#C9A84C"

                borde = f"2px solid {COLOR}" if es_hoy else f"1px solid {brd}66"
                info_f = festivos.get(fecha_dia, "")
                citas_t = f"<div style='font-size:10px;'>{num_citas} citas</div>" if num_citas > 0 else ""
                fest_t = f"<div style='font-size:8px; color:#ff6666;'>{info_f[:14]}</div>" if info_f else ""

                st.markdown(f"<div style='background:{bg}; border:{borde}; border-radius:6px; padding:6px; height:65px; text-align:center;'><strong style='color:{txt}; font-size:14px;'>{dia_num}</strong>{citas_t}{fest_t}</div>", unsafe_allow_html=True)
